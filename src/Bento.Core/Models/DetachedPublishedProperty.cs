...
namespace Bento.Core.Models {
	internal class DetachedPublishedProperty : IPublishedProperty
	{
		...
		public DetachedPublishedProperty(IPublishedPropertyType propertyType, object value, bool isPreview)
		{
			...
			_objectValue = new Lazy<object>(() => _propertyType.ConvertInterToObject(null, Umbraco.Core.PropertyEditors.PropertyCacheLevel.None, _sourceValue.Value, _isPreview));
			_xpathValue = new Lazy<object>(() => _propertyType.ConvertInterToXPath(null, Umbraco.Core.PropertyEditors.PropertyCacheLevel.None, _sourceValue.Value, _isPreview));
		}
